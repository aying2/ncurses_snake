...
void
mvwaddch_four(WINDOW *win, int y, int x, const chtype ch)
{
    int y_tf = y * 2;
    int x_tf = x * 2;
...
void
snakeview_redraw(SnakeView *view, Deque *deq, Pose food_pos)
{
    wclear(view->win);
...
InfoView *
infoview_new(int nlines, int score_ncols, int speed_ncols, int continues_ncols, int time_ncols, int begin_y, int begin_x)
{
    InfoView *info = malloc(sizeof *info);

    int ncols = score_ncols + speed_ncols + continues_ncols + time_ncols + 3 * 3 + 2;
    ...
    info->continues_win = derwin(info->win, nlines, continues_ncols, 0, midsegidx[1]);
...
void
infoview_destroy(InfoView *info)
{
...
void
infoview_update_info(InfoView *info, int score, int max_score, double speed, int continues, int time_sec)
{
    int minutes = time_sec / 60;
    int secs = time_sec % 60;
    
    int ndigs = log10(max_score) + 1;
    mvwprintw(info->score_win, 0, 0, "Score: %*d / %d", ndigs, score, max_score);
...
snakecontroller_new(int nlines, int ncols, int begin_y, int begin_x)
{
    ...
    controller->max_score = controller->model->nlines * controller->model->ncols;
    int score_ncols = (floor(log10(controller->max_score)) + 1) * 2 + SCORE_CONST_NCOLS;
    int info_ncols = score_ncols + SPEED_NCOLS + CONTINUES_NCOLS + TIME_NCOLS + 3 * 3 + 2 * 2;
    controller->info = infoview_new(1, score_ncols, SPEED_NCOLS, CONTINUES_NCOLS, TIME_NCOLS, begin_y - 3, COLS - info_ncols);
...
void
snake_controller_redraw(SnakeController *controller)
{
    snakeview_redraw(controller->view, controller->model->deq, controller->model->food_pos);
    infoview_update_info(
            controller->info, 
            controller->model->deq->length, 
            controller->max_score,
            INIT_DELAY_MS / controller->delay_ms,
...
void
snakecontroller_end_loop(SnakeController *controller)
{
    int maxy, maxx;
    ...
    WINDOW *end_border = derwin(controller->view->win, y, x, (maxy - y) / 2, (maxx - x) / 2);
    WINDOW *end_win = derwin(end_border, y - 2, x - 2, 1, 1);

    timer_pause(controller->timer);
    int score = controller->model->deq->length;
    controller->high_score = score > controller->high_score ? score : controller->high_score;
...
void
snakecontroller_help_loop(SnakeController *controller)
{
    int maxy, maxx;
    getmaxyx(controller->view->win, maxy, maxx);
    ...
    WINDOW *help_border = derwin(controller->view->win, y, x, (maxy - y) / 2, (maxx - x) / 2);
...
main(int argc, char *argv[])
{
    ...
    if(view_nlines > LINES || view_ncols > COLS
        || nlines * 2 < END_NLINES || ncols * 2 < END_NCOLS
        || nlines * 2 < HELP_NLINES || ncols * 2 < HELP_NCOLS
        || nlines <= 0 || ncols <= 0)
	{	
        endwin();
    ...
    SnakeController *controller = snakecontroller_new(nlines, ncols, 4, (COLS - ncols * 2) / 2);
